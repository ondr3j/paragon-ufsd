...
//
static const char s_FileVer[] = "$Id: ufsdjnl.c 273465 2016-04-13 12:33:16Z shura $";
...
static struct kmem_cache *jnl_free_data_cachep;
...
//  struct inode    *inode;
  ...
  //
  // Position of journal on disk (in bytes)
  //
  u64         jnl_off;
  u64         jnl_len;
};
  ...
  for ( node = rb_first( root ); NULL != node; node = rb_next( node ) ) {
    ...
    if ( NULL != prev ) {
      if ( prev->start < prev->end && prev->end < entry->start ) {
        ...
        jnl_debug( JNL_TRACE_LEVEL_FREE, "**** !!!fragment [%zx %zx) [%zx %zx)\n", prev->start, prev->end, entry->start, entry->end );
      }
    }
...
//      printk( KERN_ERR "fragment [%x %x) already on to-be-freed list\n", cluster, new_entry->end );
      printk( KERN_ERR "**** fragment [%zx %zx) already on to-be-freed list\n", entry->start, entry->end );
...
jnl_mark_bitmap(
    IN handle_j *h,
    IN int (*cb_func)( void* ctx, size_t start, size_t end ),
    IN void* ctx
    )
{
  transaction *tr       = h->tr;
    ...
    )
{
  ...
  while( NULL != n ){
    ...
    if ( entry->start >= end )
      ...
      jnl_debug( JNL_TRACE_LEVEL_FREE, "***** is_free_bitmap -> [%zx %zx) & [%zx %zx)\n", start, end, entry->start, entry->end );
      ret = 1;
      break;
    }
  }
...
//    jnl_debug( JNL_TRACE_LEVEL_COMMIT, "refile or unfile buffer b=%"PSCT"x\n", bh->b_blocknr );
  ...
  for ( ;; ) {
    ...
    jnl_debug( JNL_TRACE_LEVEL_FREE, "-fragment [%zx %zx)\n", entry->start, entry->end );
    ...
    )
{
  ...
  if ( NULL == jnl_slab[i] ) {
    ...
    jnl_slab[i] = kmem_cache_create( jnl_slab_names[i], slab_size, slab_size, 0, NULL );
    if ( NULL == jnl_slab[i] ){
...
jnl_load(
    ...
    IN void  (*commit_callback)( void  *arg, size_t start, size_t end ),
    IN void *callback_arg,
    OUT jnl **journal,
    OUT unsigned *max_transaction_buffers
    )
{
  unsigned int mask;
  int err;
...
//  j->errno  = sb->s_errno;
  ...
  j->max_transaction_buffers = *max_transaction_buffers = j->maxlen / 2;// / 4;
...
//  j->flags |= JNL_FLAGS_LOADED;
  ...
  *journal = j;
  ...
  //
  // Save position and length of journal
  //
  j->jnl_off  = off;
  j->jnl_len  = hdr->size;
...
///////////////////////////////////////////////////////////
// __blkdev_issue_fill_1
//
// generate number of 0xff filed write bios
// based on block/blk-lib.c/__blkdev_issue_zeroout
///////////////////////////////////////////////////////////
static int
__blkdev_issue_fill_1(
    ...
    )
{
  ...
  while ( 0 != nr_sects ) {
    struct bio *bio = bio_alloc( gfp_mask, min(nr_sects, (sector_t)BIO_MAX_PAGES) );
    if ( NULL == bio ) {
      ret = -ENOMEM;
      break;
    }
    ...
    while ( 0 != nr_sects ) {
//      sector_t sz  = nr_sects << 9;
//      if ( sz > PAGE_SIZE )
//        sz = PAGE_SIZE;
      sector_t sz  = (min((sector_t) PAGE_SIZE >> 9 , nr_sects)) << 9;
      ret = bio_add_page( bio, fill, sz, 0 );
      nr_sects -= ret >> 9;
      sector += ret >> 9;
      if ( ret < sz )
        break;
  ...
  if ( !atomic_dec_and_test( &bb.done ) ) {
    // 'wait_for_completion_io' for 3.9+.
    // Fix build for customized kernel where 'wait_for_completion_io' is declared but not exported
#if defined HAVE_DECL_WAIT_FOR_COMPLETION_IO && HAVE_DECL_WAIT_FOR_COMPLETION_IO && LINUX_VERSION_CODE >= KERNEL_VERSION(3,9,0)
    ...
    )
{
  int err;
  void *kaddr;
  struct page *fill = alloc_page( GFP_KERNEL  );

  if ( NULL == fill )
    return -ENOMEM;

  kaddr = atomic_kmap( fill );
  memset( kaddr, -1, PAGE_SIZE );
  atomic_kunmap( kaddr );
//  flush_dcache_page( page );

#if defined HAVE_DECL_BDEV_WRITE_SAME && HAVE_DECL_BDEV_WRITE_SAME
  if ( bdev_write_same(bdev) ) {
    unsigned char bdn[BDEVNAME_SIZE];

    err = blkdev_issue_write_same( bdev, sector, nr_sects, gfp_mask, fill );
    if ( 0 == err )
      goto out;

    pr_err("%s: WRITE SAME failed. Manually filling.\n", bdevname( bdev, bdn ) );
  }
#endif

  err = __blkdev_issue_fill_1( bdev, sector, nr_sects, gfp_mask, fill );
...
jnl_destroy(
    IN jnl *j,
    IN int fill // used by ntfs to reset $LogFile
    )
{
  int err = 0;
  ...
  if ( fill ) {
#ifdef UFSD_NTFS
    // TODO: optimize fill (if not neccessary to fill all journal)
    jnl_debug( 0, "%s: reset (-1) jnl [%llx + %llx)\n", j->devname, j->jnl_off, j->jnl_len );
    blkdev_issue_fill_1( j->dev, j->jnl_off >>9, j->jnl_len >>9, GFP_NOFS );
#else
    jnl_debug( 0, "failed to reset jnl - not implemented\n" );
    err = -EOPNOTSUPP;
#endif
  }
...
jnl_init( void )
{
  ...
  jnl_revoke_record_cache = kmem_cache_create( "jnl_revoke_record", sizeof( jnl_revoke_record ), 0, SLAB_HWCACHE_ALIGN|SLAB_TEMPORARY, NULL );
  if ( NULL == jnl_revoke_record_cache )
    return -ENOMEM;

  jnl_head_cache = kmem_cache_create( "jnl_head", sizeof( jnl_head ), 0, SLAB_TEMPORARY, NULL );
  if ( NULL == jnl_head_cache )
    goto Exit1;

  jnl_handle_cache = kmem_cache_create( "jnl_handle", sizeof( struct jnl_handle ), 0, SLAB_TEMPORARY, NULL );
  if ( NULL == jnl_handle_cache )
    goto Exit2;

  jnl_free_data_cachep = kmem_cache_create( "jnl_free_data", sizeof(jnl_free_data), 0, SLAB_RECLAIM_ACCOUNT, NULL );
  if ( NULL == jnl_free_data_cachep )
    goto Exit3;
...
jnl_exit( void )
{
  ...
  if ( NULL != jnl_free_data_cachep )
    kmem_cache_destroy( jnl_free_data_cachep );
